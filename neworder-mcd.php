...
include 'inc_email.php';
	...
	if ($_REQUEST['customerID']) //adding order
	{
		...
		$sql = "INSERT INTO tblOrders (orderType, orderStatus, createdate, duedate,  requestedby, customerID, cisTicket,  locationID,  notes,  updateemail, customerEmail, did, billProfile) VALUES ('{$_REQUEST['orderType']}', 'PENDING', now(), '{$_REQUEST['duedate']}', '{$_REQUEST['requestedby']}', {$_REQUEST['customerID']}, '{$_REQUEST['cisTicket']}', {$_REQUEST['locationID']},  '{$_REQUEST['notes']}','{$_REQUEST['updateemail']}','{$_REQUEST['customerEmail']}','{$_REQUEST['did']}','--NONE--')";
