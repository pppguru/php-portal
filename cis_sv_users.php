...
//get parent account
$sql = "SELECT * FROM KazooAccounts WHERE accountId = '{$kazooAccountID}';";
mysql_select_db($db);
...
<html lang="en">
	...
	<body class=" transparent-header container-fluid  ">
		...
		<div class="page-wrapper">
				...
				<?php 
				$sql = "select " .
                                "(SELECT count(*)  FROM KazooDevices WHERE accountId IN ('{$kazooAccountID}',(select accountId from KazooAccounts where parentAccountId = '{$kazooAccountID}')) and monitored=1 and status='DOWN') as devicesDown," .
                                "(SELECT count(*)  FROM KazooDevices WHERE accountId IN ('{$kazooAccountID}',(select accountId from KazooAccounts where parentAccountId = '{$kazooAccountID}')) and monitored=1 and status='UP') as devicesUp," .
								"(SELECT count(*)  FROM KazooDevices WHERE accountId IN ('{$kazooAccountID}',(select accountId from KazooAccounts where parentAccountId = '{$kazooAccountID}')) and monitored=0 and status !='NOT AVAILABLE' and name not like 'X_%') as notMonitored," .
								"(SELECT count(*)  FROM KazooDevices WHERE accountId IN ('{$kazooAccountID}',(select accountId from KazooAccounts where parentAccountId = '{$kazooAccountID}')) and status = ('NOT AVAILABLE'))  as devicesNA";
							//echo $sql;
							mysql_select_db($db);
							$retval = mysql_query( $sql, $conn );  
							...
							$na = $row2['devicesNA'];
							$notMonitored = $row2['notMonitored'];
				?>
						...
						<div class="row">
							<div class="col-xs-3 col-sm-6 col-md-9">
								...
								<div class="header-top-first clearfix">
									...
									<li><i class="fa fa-refresh pr-5 pl-10"></i>Status updated <?php echo $LastDeviceStatusUpdate ?> UTC
									</LI>
			...
			<!--<section class="main-container ">-->
				<div class="container-fluid">
					...
					<form class="form-inline" action="cis_sv_users.php">
						...
						<a class="btn btn-sm btn-warning" ><?php echo $notMonitored ?> Not Monitored</a> 						
						...
						<select class="form-control" name=kazooAccountID id=kazooAccountID >
							...
							$sql = "SELECT * FROM KazooAccounts where parentAccountId is null ORDER BY name";
							mysql_select_db($db);
							$retval = mysql_query( $sql, $conn );  
			...
			<!-- tabs start -->
				...
				if (!$kazooAccountID) {
					...
					$sql = "select u.*, " .
							"(select count(0) from KazooDevices where status='UP' and monitored=1 and enabled=1 " .  
								"and ownerId IN (select userId from KazooUsers  where last_name=u.last_name and KazooUsers.accountId =u.accountId)) as upDevices," .
							"(select count(0) from KazooStatusEvents where type LIKE '%RECOVERY%' and eventDate > date_sub(current_date, INTERVAL 1 DAY) " .
								"and userId IN (select userId from KazooUsers  where last_name=u.last_name and accountId =u.accountId )) as cdrEventsRecovery, " .
							"(select count(0) from KazooStatusEvents where type LIKE '%UNALLOCATED%' and eventDate > date_sub(current_date, INTERVAL 1 DAY) " .  
								"and userId IN (select userId from KazooUsers  where last_name=u.last_name and accountId =u.accountId )) as cdrEventsUnallocated, " .
							"(select count(0) from KazooDevices where status='DOWN' and monitored=1 and enabled=1 " .
								"and ownerId IN (select userId from KazooUsers  where last_name=u.last_name and accountId =u.accountId )) as downDevices," .
							"(select count(0) from KazooDevices where monitored=0 and status != 'NOT AVAILABLE' " .
								"and ownerId IN (select userId from KazooUsers  where last_name=u.last_name and accountId =u.accountId )) as notMonitoredDevices," .
							"(select count(0) from KazooDevices where status ='NOT AVAILABLE' "  .
								"and ownerId IN (select userId from KazooUsers  where last_name=u.last_name and accountId =u.accountId )) as naDevices, " .
							...
							"from KazooUsers u where accountId IN ('{$kazooAccountID}',(select accountId from KazooAccounts where parentAccountId = '{$kazooAccountID}')) and first_name not like 'X_%' group by u.last_name order by downDevices DESC, cdrEventsRecovery DESC, cdrEventsUnallocated DESC,u.last_name, u.first_name";
					...
					else
					{
						while($row = mysql_fetch_array($retval, MYSQL_ASSOC))
						{
							...
							<TR class="table-info" >
								...
								<td >
								<?php if ($upDevices > 0) { ?>
								<a class="pop btn btn-sm btn-success"  data-site="<?php echo $row['last_name'] ;?>" data-accountId="<?php echo $thisAccountId ;?>"><?php echo $upDevices ;?></a>
								<?php } if ($downDevices > 0) { ?>
								<a class="pop btn btn-sm btn-danger" data-site="<?php echo $row['last_name'] ;?>" data-accountId="<?php echo $thisAccountId ;?>"><?php echo $downDevices ;?></a>
								<?php } 
									  if ($naDevices > 0) { ?>
								<a class="pop btn btn-sm btn-info" data-site="<?php echo $row['last_name'] ;?>" data-accountId="<?php echo $thisAccountId ;?>"><?php echo $naDevices ;?></a>
								  <?php } 
								   if ($notMonitoredDevices > 0) { ?>
								<a class="pop btn btn-sm btn-warning" data-site="<?php echo $row['last_name'] ;?>" data-accountId="<?php echo $thisAccountId ;?>"><?php echo $notMonitoredDevices ;?></a>
								  <?php } 
								...
								<td><a class='btn btn-sm  btn-primary btn-animated' data-toggle='modal' data-target='#addressModal' data-accountId="<?php echo $thisAccountId ;?>" onclick='javascript:get_911(<?php echo $callerid;?>,<?php echo $thisSite;?>)'>911 Address <i class="fa fa-map-marker"></i></a>&nbsp;<a class='btn btn-sm  btn-primary btn-animated' data-toggle='modal' data-target='#chartModal' onclick='javascript:get_charts1("<?php echo $thisSite;?>","<?php echo $tz;?>","<?php echo $thisAccountId ;?>")'>Call Analytics <i class="fa fa-bar-chart"></i></a>&nbsp;<a  type="button" class="btn btn-sm btn-primary btn-animated" data-toggle='modal' data-target='#speedModal' data-accountId="<?php echo $thisAccountId ;?>" onclick='javascript:get_speedtests(<?php echo $last_name;?>)'>Speed Tests<i class="fa fa-fighter-jet"></i></a></td>
			...
			<div class="modal-dialog" >
				<div class="modal-content">
					...
					<div class="modal-footer">
						...
						<div id=reboot></div>
						<input type=hidden id=deviceId value=''>=<input type=hidden id=accountId value=''>
		...
		<div class="modal modal-wide fade" id="configModal"  tabindex="-1" role="dialog" aria-labelledby="configModalLabel" aria-hidden="true">
			<div class="modal-dialog" >
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
						<h4 class="modal-title" id="configModalLabel"></h4><div id=div_lastAccess></div>
					</div>
					...
					<div class="modal-body" id="configBody">
						...
						<INPUT name=deviceName id=deviceName type=hidden>
							...
							<div class="form-group has-feedback" id="base_main">
							...
							<div class="form-group has-feedback" id="transport_main">
							...
							<div class="form-group has-feedback" id="proxy_main">
							...
							<div class="form-group has-feedback" id="refresh_main">
							...
							<div class="form-group has-feedback">
								...
								<div class="col-sm-8">
                                    ...
                                    <button type=button id=updateConfig class='btn radius-10 btn-danger btn-sm ' onclick="javascript:remove_config()">REMOVE Config</button>
								</div>
							</div>
		...
		$(document).ready(function () {
			...
			$('.collapse').on('show.bs.collapse', function () {
			 ...
			 $('.pop').popover({
					...
					content: function () {
                        ...
                        document.getElementById('accountId').value = $(this).attr('data-accountId');
						$.ajax({
							type: 'GET',
							url: "ajax_functions.php?fn=get-site-devices&last_name="+$(this).attr('data-site')+"&accountId="+$(this).attr('data-accountId'),
			...
			function get_events(deviceId, accountId) {
				thediv = '#events';
				//alert(deviceId);
				...
				$.ajax({
					type: "GET",
					url: "ajax_functions.php?fn=get_monitor&deviceId="+deviceId,
			...
			function toggle_monitor(deviceId,monitored) {
				thediv = '#monitored';
				...
				$.ajax({
					type: "GET",
					url: "ajax_functions.php?fn=toggle_monitor&deviceId="+deviceId+'&monitored='+monitored,
			...
			function reboot_phone() {
				...
				$.ajax({
					type: "GET",
					url: "api_kazoo/kazoo_reboot_phone.php?accountId="+accountId+"&deviceId="+deviceId,
			...
			*/
			...
			function get_userlist(last_name) {
                ...
                $.ajax({
					type: "GET",
					url: 'ajax_functions.php?fn=get-userlist&accountId='+accountId+'&last_name='+last_name,
			...
			function get_charts1(site,timezone,accountId) {
			...
			function get_charts2() {
				...
				$.ajax({
					type: "GET",
					url: "cdr-charts/chart-"+charttype+".php?charttype="+document.getElementById('charttype').value+"&site="+document.getElementById('site').value+"&timezone="+document.getElementById('timezone').value+"&startdate="+document.getElementById('startdate').value+"&enddate="+document.getElementById('enddate').value+"&account_id="+accountId,
			...
			function get_config(deviceId,deviceName, userId,siteNumber) {
				...
				//Hide for non SV users
                <?php if ($_SESSION['roleInfo'] > 2) { ?>
                $('#codec_main').hide();
                $('#transport_main').hide();
                $('#proxy_main').hide();
                $('#refresh_main').hide();
				...
				$.ajax({
					...
					success: function(data){
						...
						if (data['result'] == 'NOT FOUND') {
                            ...
                            $('#div_lastAccess').html(data['lastAccess']);
                            customerTemplateID = data['customerTemplateID'];
							
						}

						get_config_options(customerTemplateID,data['codec'],data['transport'],data['proxy']);

						get_devices_dd(deviceId,data['line2'],data['line3']);

                        show_hide_device_dd(customerTemplateID);
			...
			function get_config_options(customerTemplateID, codec, transport, proxy) {
				...
				accountId = '<?php echo $kazooAccountID ?>';
				//alert(accountId);
				$.ajax({
					type: "GET",
					url: "ajax_functions.php?fn=get_config_options&accountId="+accountId+"&customerTemplateID="+customerTemplateID+"&codec="+codec+"&transport="+transport+"&proxy="+proxy,
					dataType:'json',
					success: function(data){
			...
			function show_hide_device_dd(customerTemplateID){
			    ...
			    if (customerTemplateID=='') {
                    ...
                    $.ajax({
                        type: "GET",
                        url: "ajax_functions.php?fn=get-devicenumber&customerTemplateID=" + customerTemplateID,
                        dataType: 'json',
                        success: function (data) {
			...
			function get_devices_dd(deviceId,line2,line3) {
				...
				$.ajax({
					type: "GET",
					url: "ajax_functions.php?fn=get-devices&siteNumber="+siteNumber+"&deviceId="+deviceId+"&line2="+line2+"&line3="+line3+"&accountId="+accountId,
					dataType:'json',
					success: function(data){
			...
			function remove_config() {
                ...
                if (confirm('Are you sure you want to permanently delete the config for mac: '+mac+'?')) {
                    $.ajax({
                        type: "GET",
                        url: "ajax_functions.php?fn=remove_config&mac=" + mac,
			...
			function update_config() {
				...
				if (lineError){ 
					...
					if (mac.length!== 12) {
						...
						if (mac && customerTemplateID && codec && transport && line1 && proxy) {
						
							$('#configUpdateMsg').html('Updating Config... <i class="fa fa-refresh fa-spin" style="font-size:24px"></i>');

							$.ajax({
								type: "GET",
								url: "ajax_functions.php?fn=update_config&line1="+line1+"&line2="+line2+"&line3="+line3+"&mac="+mac+"&accountId=<?php echo $kazooAccountID?>&customerTemplateID="+customerTemplateID+"&codec="+codec+"&transport="+transport+"&proxy="+proxy,
