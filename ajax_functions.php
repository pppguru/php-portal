...
//ini_set('display_errors', 'On');
	...
	if ($fn == 'notes') {
        ...
        $sql = "Update tblOrders set  orderStatus='IN PROGRESS' where orderStatus='PENDING' and orderID = {$_REQUEST['orderID']}";
        mysql_select_db($db);
        $retval = mysql_query( $sql, $conn );
	...
	if ($fn == "get_config_info") {
		...
		else
		{
            ...
            $lastAccess = "Last Accessed: " . $row['lastAccess'] . " CST";
						
			$arr = array ('result'=> 'SUCCESS','phoneModelID'=>$phoneModelID,'mac'=> $mac,'baseTemplateID'=> $baseTemplateID,'customerTemplateID'=>$customerTemplateID,'codec'=>$codec,'transport'=>$transport,'proxy'=>$proxy,'line2'=>$line2,'line3'=>$line3, 'lastAccess'=>$lastAccess);
	...
	if ($fn == "update_config") {
		...
		$sql = "DELETE FROM apCONFIGS where deviceID='{$line1}'";
		mysql_select_db($db);
		$retval = mysql_query( $sql, $conn );
		//echo $sql;
		$sql = "INSERT INTO apCONFIGS (proxy, mac, phoneModelID, customerTemplateID, codec, transport, accountId, deviceID, deviceID_line2, deviceID_line3, deviceID_line4, lastUpdate) VALUES ('{$proxy}','{$mac}', (select phoneModelID from apTemplates where templateID={$customerTemplateID}), {$customerTemplateID}, '{$codec}', '{$transport}', '{$accountId}', '{$line1}', '{$line2}', '{$line3}','{$line4}','{$now}') ";
		//echo $sql;
		mysql_select_db($db);
		$retval = mysql_query( $sql, $conn );
        //echo $sql;
	...
	if ($fn == "get_config_options") {
		...
		/*
		$sql = "select * from apPhoneModels where phoneModelID IN (select phoneModelID from apTemplates where type='CUSTOMER' and accountId='{$accountId}') order by phoneModel";
		mysql_select_db($db);
		$retval = mysql_query( $sql, $conn );
        ...
        */
        ...
        $sql = "select t.*, (select phoneModel from apPhoneModels where phoneModelID=t.phoneModelID) as baseTemplateName from apTemplates t where t.type='CUSTOMER' and t.accountId='{$accountId}'";
		//echo $sql;
        if ($acct=='X_') {
			$sql = "select t.*,(select phoneModel from apPhoneModels where phoneModelID=t.phoneModelID) as baseTemplateName from apTemplates t where  t.type='CUSTOMER' order by t.accountId";
		}
		mysql_select_db($db);
		$retval = mysql_query( $sql, $conn );

        if(mysql_num_rows($retval) > 0) {
            $customer = "<select name=customerTemplateID id=customerTemplateID required oninput='javascript:show_hide_device_dd(this.options[this.selectedIndex].value)'>";
            $customer .= "<option value=''>--PLEASE SELECT--</option>";
            while ($row = mysql_fetch_array($retval, MYSQL_ASSOC)) {
                $sel = '';
                ...
                */
                ...
                $customer .= "<option {$sel} value='" . $row['templateID'] . "'>" . $row['baseTemplateName'] . ": " .$row['templateName'] . "</option>";
            }
            $customer .= "</select>";
        }
		$phoneName = $row['baseTemplateName'];
		...
		$arr = array ('customer'=> $customer,'codec'=> $codecstr,'transport'=>$transportstr, 'proxy'=>$proxystr, 'phoneName'=>$phoneName);
		echo json_encode($arr);
	}
	if ($fn == "get-devicenumber") {
        $customerTemplateID = $_REQUEST['customerTemplateID'];
        $sql = "select devices from apTemplates where templateID={$customerTemplateID}";
        mysql_select_db($db);
        $retval = mysql_query( $sql, $conn );
    ...
    else if ($fn == "set-template") {
        $templateID = $_REQUEST['templateID'];
        $templateName = $_REQUEST['templateName'];
        $configText = $_REQUEST['configText'];
        $isDefaultOption = $_REQUEST['isDefaultOption'];
        $accountID = $_REQUEST['accountID'];
        $now = date("Y-m-d");

        $sql = "UPDATE apTemplates SET templateName = '{$templateName}', config = '{$configText}', isDefault={$isDefaultOption}, lastUpdated = '{$now}' WHERE templateID={$templateID}";

        mysql_select_db($db);
        $retval = mysql_query( $sql, $conn );

        if ($retval) { //Success
            $s = "Template updated. Please close the window.";

            if ($isDefaultOption == 1 && $accountID != '0'){  //Update other records with default 0 value
                $sql = "UPDATE apTemplates SET isDefault=0 WHERE accountId='{$accountID}' AND templateID!={$templateID}";
	...
	if ($fn == "get-devices") {
		...
		if(mysql_num_rows($retval) > 0) {
            $line1 = "<input type='hidden' name=line1 id=line1 value='{$deviceId}'>";
            $line2 = "<select name=line2 id=line2 required >";
            $line3 = "<select name=line3 id=line3 required >";

            $line2 .= "<option selected value=''>--PLEASE SELECT--</option>";
            $line3 .= "<option selected value=''>--PLEASE SELECT--</option>";
	...
	if ($fn == "get-site-devices") {
		...
		$sql = "SELECT d.*, (select count(0) from KazooStatusEvents where type LIKE '%RECOVERY%' and eventDate > date_sub(current_date, INTERVAL 1 DAY) and deviceId=d.deviceId) as cdrEventsRecovery,  " .  
					" (select count(0) from KazooStatusEvents where type LIKE '%UNALLOCATED%' and eventDate > date_sub(current_date, INTERVAL 1 DAY) and deviceId=d.deviceId) as cdrEventsUnallocated, (select network_ip from KazooRegistrations r where r.deviceId = d.deviceId ) as network_ip, (select user_agent from KazooRegistrations r where r.deviceId = d.deviceId ) as user_agent, (select network_port from KazooRegistrations r where r.deviceId = d.deviceId ) as network_port, (select count(0) from apCONFIGS where deviceID=d.deviceId) as configs, (select count(0) from apCONFIGS where deviceID_line2=d.deviceId or deviceID_line3=d.deviceId) as configs2 FROM KazooDevices d where ownerId IN (select userId from KazooUsers where accountId = '" . $accountId . "' and last_name='" . $last_name . "') order by name  ";
		//echo $sql;
		mysql_select_db($db);
		$retval = mysql_query( $sql, $conn );  
		if(mysql_num_rows($retval) == 0)
		...
		else
		{
			...
			while($row = mysql_fetch_array($retval, MYSQL_ASSOC))
			{
				...
				if ($monitored==0){
					$icon = "<i class='fa fa-toggle-off fa-2x text-muted fa-fw' data-toggle='tooltip' title='Not Monitored'></i> ";
				}else {
					if ($status=='DOWN') {
						//not registered
                ...
                if ( $_SESSION['roleInfo'] <=3) {


                    if ($config2 == 0) {  //only config from the main line
                        if ($config > 0) {
                        ...
                        $configIcon = "<a href='#' data-toggle='modal' data-target='#configModal' data-accountId='{$accountId}' onclick='javascript:get_config(\"" . $row['deviceId'] . "\",\"" . $name . "\",\"" . $row['ownerId'] . "\",\"" . $last_name . "\",\"" . $accountId . "\")'>" . $cfIcon . "</a>";
                    }
                }
			$s .= "<tr><td>" . $configIcon . "</td><td align=left nowrap><a href='#' data-toggle='modal' data-target='#eventModal' data-accountId='{$accountId}' onclick='javascript:get_events(\"" . $row['deviceId'] . "\",\"" . $accountId . "\")'>" . $icon . "{$cdrIcon} {$cdrIcon2}</a></td><TD align=left>"  . $row['name'] . "</td><TD nowrap align=left>&nbsp;&nbsp;&nbsp;&nbsp;"  . $network_ip . ":" . $network_port . "</td><td nowrap>" . substr($user_agent,0,60) . "</td><td><a href='#' data-toggle='modal' data-target='#jsonModal' onclick='javascript:get_json(\"" . $row['deviceId'] . "\")'>JSON</a></td></tr>";
			}
			$s .= "</table>";
		}
	...
	if ($fn == "get_monitor") {
		...
		$sql = "SELECT * from KazooDevices WHERE deviceId='{$deviceId}'";
		...
		if ($retval) {
			$row = mysql_fetch_array($retval, MYSQL_ASSOC);
			if ($row['monitored'] == 1) {
				$s = "<a href='#' onclick='javascript:toggle_monitor(\"" . $deviceId . "\",0)'><i class='icon-thumbs-up fa-2x text-success'></i> Monitored</a>";
			} else {
				$s = "<a href='#' onclick='javascript:toggle_monitor(\"" . $deviceId . "\",1)'><i class='icon-thumbs-down fa-2x text-danger'></i> NOT Monitored</a>";
			}
		} else {
			$s = "ERROR!";
		}
	...
	if ($fn == "toggle_monitor") {
		
		$deviceId = $_REQUEST['deviceId'];
		$monitored = $_REQUEST['monitored'];
		
		
		$sql = "UPDATE KazooDevices SET monitored = {$monitored} WHERE deviceId='{$deviceId}'";
