...
include "inc_db.php";
	...
	while($row = mysql_fetch_array($devices, MYSQL_ASSOC)) {
		...
		$diff=abs(strtotime($LastUpdate) - strtotime($LastRegistered));
		...
		if (substr($customer,0,1) != 'X_') {
			...
			else 
			{
				...
				if ($diff <= 600 AND ($status=='DOWN')) {
					...
					$downtime=round(abs(strtotime($eventDate) - strtotime($lastDown))/60);
				...
				if ($diff <= 600 AND ($status=='' OR $status=='NOT AVAILABLE')) {
					...
					$sqlstatus = "UPDATE KazooDevices SET status='UP', monitored=1 WHERE deviceID='{$deviceId}'";
					$eventCount++;
				}
