   ...
   $retval = mysql_query( $sql, $conn );  
	...
	else
	{
		...
		<tr>
			...
			<td nowrap>
						<?php if ($_SESSION['roleInfo'] <= 2  AND $row['orderType'] !== "LNP" and $row['didType'] == "PORT REQUESTED" AND (int)$row['lnpCount'] == 0  AND ($row['orderStatus'] == "ACTIVATED" OR $row['orderStatus'] == "BILLED")) {?>
				<a href='neworder-lnp1.php?orderID=<?php echo $row['orderID'] ;?>'  type="button" class="btn btn-group btn-sm btn-info btn-animated">Port In<i class="fa fa-phone"></i></a>					
			<?php } ?>
