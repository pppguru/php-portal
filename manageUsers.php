<?php include 'inc_header.php';
	...
	if ($_REQUEST['action'] == 'addMember') {   // --------------------- Register new member------------------------- //
        ...
        if($firstResult = mysql_fetch_array($result, MYSQL_ASSOC)) {
        	echo "<div class='alert alert-danger cm'><strong>Failed!</strong> Already existing email address : " . $emailAddr . "</div>";
		}
		else {
			$query = "INSERT INTO tblUsers (email, password, userTypeId) VALUES ('{$emailAddr}', md5('{$password}'), {$role})";
			mysql_query($query, $conn);

			echo "<div class='alert alert-success cm'><strong>Success!</strong> Successfully added the new member : " . $emailAddr . "</div>";	
		}
	}
	else if ($_REQUEST['action'] == 'removeMember') {   // --------------------- Register new member------------------------- //
        ...
        if($result) {
        	echo "<div class='alert alert-success cm'><strong>Success!</strong> Successfully removed the member.</div>";	
        	
		}
		else {
			echo "<div class='alert alert-danger cm'><strong>Failed!</strong> Failed to remove the member.</div>";
		}
	}
...
<html lang="en">
	...
	<head>
		<meta charset="utf-8">
		<title>SimpleVoIP Manage Users(Admin)</title>
	...
	<body class="no-trans   ">
		...
		<div class="page-wrapper">
			...
			<section class="main-container padding-bottom-clear">
				<div class="container-fluid">
					...
					<!-- ================ -->
					<div class="main">
						<h1>SimpleVoIP Manage Users</h1>
						<!-- page-title start -->
						<!-- ================ -->
						<a href='#' data-target="#signUpModal" data-toggle="modal" type="button" class="btn btn-group btn-sm btn-success btn-animated">Add New Member <i class="fa fa-plus"></i>
						</a>
		...
		<section>
			...
			<div class="row">
				   ...
				   $sql = "SELECT tblUsers.userId as userID, tblUsers.email as email, tblUserRoles.roleName as roleName FROM tblUsers, tblUserRoles where tblUsers.userTypeId = tblUserRoles.userRoleId AND tblUserRoles.roleName != 'ADMIN'";  //Fetch all users without ADMIN
				   ...
				   if(! $retval )
				   {
					...
					?>
						<tr>
							<td><?php echo $numNo;?></td>
							<td><?php echo $row['email'];?></td>
							<td><?php echo $row['roleName'];?></td>
							<td>
								<div id="actions">
									<form action="manageUsers.php" method=post style="margin: 0px 0;">
